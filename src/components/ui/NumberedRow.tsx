      ...
      {items.map((item, index) => (
        <li key={item.num} className="border-b border-white/10">
          <motion.div
            ...
            className="grid gap-6 py-8 md:grid-cols-12 md:gap-8 md:py-12"
          >
            <span className="mono col-span-2 text-xs uppercase tracking-[0.2em] text-neutral-500 md:col-span-1">
              /{item.num}
            </span>
            <h3 className="display-md text-white md:col-span-4">{item.title}</h3>
            <p className="text-base leading-relaxed text-neutral-400 md:col-span-6">
              {item.text}
            </p>
            {item.meta && (
              <span className="mono text-[10px] uppercase tracking-[0.2em] text-neutral-600 md:col-span-1 md:text-right">
                {item.meta}
              </span>
            )}
          </motion.div>
